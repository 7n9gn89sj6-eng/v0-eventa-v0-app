...
export default async function EditEventPage({ params }: EditPageProps) {
  ...
  // ----------------------------
  // Extract token from Render headers
  // ----------------------------
  const h = headers();
  const orig = h.get("x-original-url");
  const referer = h.get("referer");

  // This gives us something like: "/edit/123?token=ABC123"
  const rawUrl = orig || referer || "";

  // Create a fake base URL because new URL() requires absolute URLs
  const url = new URL(rawUrl, "https://example.com");

  // Pull token (case-insensitive)
  const token =
